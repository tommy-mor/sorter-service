...
(defn logout [env]
  (-> env
    ...
    (uism/trigger-remote-mutation :actor/login-form 'ssorter.model.users/logout {})
    (uism/activate :state/logged-out)))

(defn login [{::uism/keys [event-data] :as env}]
  (-> env
    (clear)
    (uism/trigger-remote-mutation :actor/login-form 'ssorter.model.users/login
