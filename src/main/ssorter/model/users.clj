...
(pco/defresolver current-session-resolver [env input]
  ...
  (let [{:keys [:users/user_name session/valid?]} (get-in env [:ring/request :session])]
    (if valid?
      (do
        (log/info user_name "already logged in!")
        {::current-session {:session/valid? true :users/user_name user_name}})
...
(pco/defmutation login [env {:keys [username password]}]
  ...
  (let [hash (-> (h/select :password_hash)
    ...
    (if (and hash (password/check password hash))
